...
def escolher_poltronas(sala, quantidade, poltronas):
    ...
    while cont < quantidade:
        ...
        if poltronas[sala][lugar - 1] == 'D':
            print("Poltrona disponível\n")
            poltronas[sala][lugar - 1] = 'I'
...
while menu != 8:
    ...
    if menu == 1:
        exibir_poltronas("F1_S1")
        info(filmes_assentos, "F1_S1", precos, 0, 1, 2)
        quantidade = int(input("Quantas poltronas deseja selecionar? "))
        poltronas_escolhidas = escolher_poltronas("F1_S1", quantidade, poltronas)
        for i in poltronas_escolhidas:
            ingresso = int(input(f"\n Escolha o tipo de ingresso para a poltrona {i}: \n 1. Inteira = R${precos[0]} \n 2. Meia = R${precos[1]} \n 3. VIP = R${precos[2]}\n"))
            if ingresso == 1:
                calculos(precos, 0, qtd_ingressos, valores_arrecadados, "F1_S1", "Inteira", 1)
            elif ingresso == 2:
                calculos(precos, 1, qtd_ingressos, valores_arrecadados, "F1_S1", "Meia", 1)
            elif ingresso == 3:
                calculos(precos, 2, qtd_ingressos, valores_arrecadados, "F1_S1", "VIP", 1)
            else:
                print("Tipo de ingresso inválido.")
                filmes_assentos["F1_S1"] += 1
        filmes_assentos["F1_S1"] -= quantidade

    if menu == 2:
        exibir_poltronas("F1_S2")
        info(filmes_assentos, "F1_S2", precos, 0, 1, 2)
        quantidade = int(input("Quantas poltronas deseja selecionar? "))
        poltronas_escolhidas = escolher_poltronas("F1_S2", quantidade, poltronas)
        for i in poltronas_escolhidas:
            ingresso = int(input(f"Escolha o tipo de ingresso para a poltrona {i}: \n 1. Inteira = R${precos[0]} \n 2. Meia = R${precos[1]} \n 3. VIP = R${precos[2]}\n"))
            if ingresso == 1:
                calculos(precos, 0, qtd_ingressos, valores_arrecadados, "F1_S2", "Inteira", 1)
            elif ingresso == 2:
                calculos(precos, 1, qtd_ingressos, valores_arrecadados, "F1_S2", "Meia", 1)
            elif ingresso == 3:
                calculos(precos, 2, qtd_ingressos, valores_arrecadados, "F1_S2", "VIP", 1)
            else:
                print("Tipo de ingresso inválido.")
                filmes_assentos["F1_S2"] += 1
        filmes_assentos["F1_S2"] -= quantidade

    if menu == 3:
        exibir_poltronas("F2_S1")
        info(filmes_assentos, "F2_S1", precos, 3, 4, 5)
        quantidade = int(input("Quantas poltronas deseja selecionar? "))
        poltronas_escolhidas = escolher_poltronas("F2_S1", quantidade, poltronas)
        for i in poltronas_escolhidas:
            ingresso = int(input(f"Escolha o tipo de ingresso para a poltrona {i}: \n 1. Inteira = R${precos[3]} \n 2. Meia = R${precos[4]} \n 3. VIP = R${precos[5]}\n"))
            if ingresso == 1:
                calculos(precos, 3, qtd_ingressos, valores_arrecadados, "F2_S1", "Inteira", 1)
            elif ingresso == 2:
                calculos(precos, 4, qtd_ingressos, valores_arrecadados, "F2_S1", "Meia", 1)
            elif ingresso == 3:
                calculos(precos, 5, qtd_ingressos, valores_arrecadados, "F2_S1", "VIP", 1)
            else:
                print("Tipo de ingresso inválido.")
                filmes_assentos["F2_S1"] += 1
        filmes_assentos["F2_S1"] -= quantidade

    if menu == 4:
        exibir_poltronas("F2_S2")
        info(filmes_assentos, "F2_S2", precos, 3, 4, 5)
        quantidade = int(input("Quantas poltronas deseja selecionar? "))
        poltronas_escolhidas = escolher_poltronas("F2_S2", quantidade, poltronas)
        for i in poltronas_escolhidas:
            ingresso = int(input(f"Escolha o tipo de ingresso para a poltrona {i}: \n 1. Inteira = R${precos[3]} \n 2. Meia = R${precos[4]} \n 3. VIP = R${precos[5]}\n"))
            if ingresso == 1:
                calculos(precos, 3, qtd_ingressos, valores_arrecadados, "F2_S2", "Inteira", 1)
            elif ingresso == 2:
                calculos(precos, 4, qtd_ingressos, valores_arrecadados, "F2_S2", "Meia", 1)
            elif ingresso == 3:
                calculos(precos, 5, qtd_ingressos, valores_arrecadados, "F2_S2", "VIP", 1)
            else:
                print("Tipo de ingresso inválido.")
                filmes_assentos["F2_S2"] += 1
        filmes_assentos["F2_S2"] -= quantidade

    if menu == 5:
        exibir_poltronas("F3_S1")
        info(filmes_assentos, "F3_S1", precos, 1, 6, 3)
        quantidade = int(input("Quantas poltronas deseja selecionar? "))
        poltronas_escolhidas = escolher_poltronas("F3_S1", quantidade, poltronas)
        for i in poltronas_escolhidas:
            ingresso = int(input(f"Escolha o tipo de ingresso para a poltrona {i}: \n 1. Inteira = R${precos[1]} \n 2. Meia = R${precos[6]} \n 3. VIP = R${precos[2]}\n"))
            if ingresso == 1:
                calculos(precos, 1, qtd_ingressos, valores_arrecadados, "F3_S1", "Inteira", 1)
            elif ingresso == 2:
                calculos(precos, 6, qtd_ingressos, valores_arrecadados, "F3_S1", "Meia", 1)
            elif ingresso == 3:
                calculos(precos, 2, qtd_ingressos, valores_arrecadados, "F3_S1", "VIP", 1)
            else:
                print("Tipo de ingresso inválido.")
                filmes_assentos["F3_S1"] += 1
        filmes_assentos["F3_S1"] -= quantidade

    if menu == 6:
        exibir_poltronas("F3_S2")
        info(filmes_assentos, "F3_S2", precos, 1, 6, 3)
        quantidade = int(input("Quantas poltronas deseja selecionar? "))
        poltronas_escolhidas = escolher_poltronas("F3_S2", quantidade, poltronas)
        for i in poltronas_escolhidas:
            ingresso = int(input(f"Escolha o tipo de ingresso para a poltrona {i}: \n 1. Inteira = R${precos[1]} \n 2. Meia = R${precos[6]} \n 3. VIP = R${precos[2]}\n"))
            if ingresso == 1:
                calculos(precos, 1, qtd_ingressos, valores_arrecadados, "F3_S2", "Inteira", 1)
            elif ingresso == 2:
                calculos(precos, 6, qtd_ingressos, valores_arrecadados, "F3_S2", "Meia", 1)
            elif ingresso == 3:
                calculos(precos, 2, qtd_ingressos, valores_arrecadados, "F3_S2", "VIP", 1)
            else:
                print("Tipo de ingresso inválido.")
                filmes_assentos["F3_S2"] += 1
